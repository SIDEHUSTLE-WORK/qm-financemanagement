...
// EGO SMS Configuration from environment variables
...
const formatPhoneNumber = (phone) => {
  ...
  if (cleaned.startsWith('+256')) {
    ...
    cleaned = '256' + cleaned.substring(1); // Replace leading 0 with 256
  } else if (!cleaned.startsWith('256')) {
    cleaned = '256' + cleaned; // Add 256 prefix
  }
...
router.post('/send-bulk', authenticateToken, async (req, res) => {
  try {
    const { messages } = req.body; // Array of { phone, message, studentId, studentName }
